...
from utils.utils import get_device, get_loaders, get_loss_function, get_transforms, load_checkpoint, get_metrics
from loggers.logs import log_calib
def main(config):
    device = get_device(config)
    assert config.load.path, print(f'checkpoint path was not provided')
    ...
    print(global_metrics, label_metrics, sep='\n')
    ...
    logits_calib, labels_calib = predict(model, val_loader, loss_fn, device)
    logits_flatten = flatten_logits(logits_calib)
    scores = softmax_tensor_to_numpy(logits_flatten)
    labels_flatten = labels_calib.flatten().cpu().numpy()
    
    plot_results(gscv, scores, labels_flatten, NOW, odir)

    
    log_calib(labels_calib, logits_calib, global_metrics, label_metrics, config, gscv, device)
