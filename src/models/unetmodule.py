...
class UNET(nn.Module):
    def __init__(self, config: Union[DictConfig, ListConfig]):
        ...
        self.double_conv_downs = nn.ModuleList(
            [self.__double_conv(layer, layer_n)
             for layer, layer_n in zip(self.layers[:-1],
                                       self.layers[1:])]
            )

        self.up_trans = nn.ModuleList(
            [nn.ConvTranspose2d(layer, layer_n, kernel_size=2, stride=2)
             for layer, layer_n in zip(self.layers[::-1][:-2],
                                       self.layers[::-1][1:-1])]
            )
        ...
        self.final_conv = nn.Conv2d(self.min_layer_size,
                                    classes,
                                    kernel_size=1)
    ...
    def forward(self, x):
        ...
        # up layers
        for up_trans, double_conv_up, concat_layer in zip(self.up_trans,
                                                          self.double_conv_ups,
                                                          concat_layers):
            x = up_trans(x)
...
class UNETModule(pl.LightningModule):
    ...
    def get_criterion(self):
        self.loss_fn = load_obj(
            self.cfg.criterion.class_name
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.validation_dict = {'images': images,
                                'targets': targets,
    ...
    def wandb_image_dict(self, log_dict):
        for image, target, prediction, idx in zip(
            ...
            wandb_image = wandb.Image(
                image=local_image,
                masks={"prediction": {"mask_data": local_prediction,
                                      "class_labels": self.cfg.dataset.labels
                                      },
                       "ground truth": {"mask_data": local_target,
                                        "class_labels": self.cfg.dataset.labels
                                        }
                       }
                                      )
