...
def train(cfg: str):
    ...
    logging.basicConfig(filename=cfg.general.logging_file,
                        encoding='utf-8',
                        level=logging.DEBUG)

    try:
        assert os.path.isfile(cfg.dataset.params.train_csv)
    except AssertionError as err:
        logging.exception('''insert a valid path in train_csv at
    ...
    data_module = load_obj(cfg.dataset.class_name)(cfg)
    data_module.setup('training')
    logging.info(f'data module {cfg.dataset.class_name} was set.')
    ...
    setattr(cfg.trainer,
            'log_every_n_steps',
            len(data_module.val_dataset) // cfg.dataset.params.batch_size)

    logger = [load_obj(logger['class_name'])(**logger['params'])
              for logger in cfg.logger]

    [logging.info(f'logger {logger["class_name"]} was set')
     for logger in cfg.logger]

    callbacks = [load_obj(call['class_name'])(**call['params'])
...
if __name__ == '__main__':
    ...
    if not WANDB_API_KEY:
        WANDB_API_KEY = getpass('WANDB_ML API_KEY: ')
