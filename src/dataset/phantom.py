...
class PhantomDataset(Dataset):
    def __init__(
        ...
        self.image_names = [os.path.splitext(os.path.basename(image)) for image in self.images]
...
class PhantomDCMDataset(PhantomDataset):
    ...
    def __getitem__(self, idx: int) -> tuple[Tensor, Tensor, tuple[int, str]]:
        ...
        return image, mask, (idx, self.images[idx])
...
class PhantomTIFFDataset(PhantomDataset):
    def __getitem__(self, idx: int) -> tuple[Tensor, Tensor, tuple[int, str]]:
        ...
        # Convert Image and Mask to numpy
        image = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE).astype(cv2.CV_32FC1)
        mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE).astype(cv2.CV_8UC1)
