...
class ConfidenceHistogram(metrics.MaxProbCELoss):
    def plot(self,
        ...
        ticks = np.arange(0, 1.2, .2).tolist()
        plt.xticks(ticks, map(str, ticks))
        plt.yticks(ticks, map(str, ticks))

        # plot grid
        plt.grid(color='tab:grey', linestyle=(0, (1, 5)),
        ...
        if acc > conf:
            plt.text(acc + .03, 0.9, 'Accuracy',
                     rotation=90, fontsize=11)
            plt.text(conf - .07, 0.9, 'Avg. Confidence',
                     rotation=90, fontsize=11)
        else:
            plt.text(acc - .07, .9, 'Accuracy',
                     rotation=90, fontsize=11)
            plt.text(conf + .03, .9, 'Avg. Confidence',
                     rotation=90, fontsize=11)
...
class ReliabilityDiagram(metrics.MaxProbCELoss):
    def plot(self,
        ...
        plt.bar(x, self.bin_acc, color='b',
                ...
                edgecolor='k', label='Outputs',
                zorder=5)
        plt.bar(x, error,
                bottom=np.minimum(self.bin_acc, mid),
                color='mistyrose', alpha=0.5,
                width=delta, align='edge', edgecolor='r',
                hatch='/', label='Gap', zorder=10)

        ident = [0.0, 1.0]
        plt.plot(ident, ident, linestyle='--',
                 color='tab:grey', zorder=15)

        # labels and legend
        plt.ylabel('Accuracy', fontsize=13)
        plt.xlabel('Confidence', fontsize=13)
        plt.legend(loc='upper left',
                   framealpha=1.0, fontsize='medium')
        if title is not None:
            plt.title(title, fontsize=16)
        plt.tight_layout()
