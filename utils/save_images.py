...
def save_predictions_as_imgs(data,
                             label,
                             predictions,
                             config,
                             step,
    ...
    for j in range(preds_labels.size(0)):
        ...
        wandb_image = wandb_mask(local_data,
                                 local_label,
                                 config.image.labels,
                                 local_pred)
...
def save_calib(model, predictions, config):
    calib_path = os.path.join(config.paths.predictions_dir_pos_calib,
                              config.project.time)
    make_dir(calib_path)
    ...
    return (torch.nn.functional.softmax(predictions, 1),
...
def save_validation_as_imgs(loader, config):
    ...
    with torch.no_grad():
        for idx, (x, y) in enumerate(loader):
            img = os.path.join(config.paths.predictions_dir,
    ...
    for p, preds in enumerate(predictions):
        for idx, label in enumerate(preds):
            path = os.path.join(img_path,
                                f'confidence_{name}_{p}_{idx}.png')
