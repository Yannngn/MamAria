...
def get_weights(config):
    ...
    total_pixels = 0
    mask_files = [os.path.join(mask_dir, file)
...
def get_loss_function(config,
                      loss_list=[None, 'crossentropy', 'tversky', 'focal']
                      ):
...
def get_scheduler(config,
                  optimizer,
                  scheduler_list=['plateau', 'cosine', 'cyclic', 'warm']
                  ):
    ...
    try:
        ...
        logging.error(f'''{scheduler_fn} is not a recognized
...
def get_metrics(config):
    ...
    global_metrics = [
        ...
        torchmetrics.Accuracy(num_classes=num_classes,
                              average=average,
                              mdmc_average='global').to(device),
        torchmetrics.F1Score(num_classes=num_classes,
                             average=average,
                             mdmc_average='global').to(device),
        torchmetrics.Precision(num_classes=num_classes,
                               average=average,
                               mdmc_average='global').to(device),
        torchmetrics.Recall(num_classes=num_classes,
                            average=average,
                            mdmc_average='global').to(device),
        torchmetrics.Specificity(num_classes=num_classes,
                                 average=average,
                                 mdmc_average='global').to(device),
        torchmetrics.JaccardIndex(num_classes=num_classes,
                                  average=average,
                                  mdmc_average='global').to(device)
        ]

    global_metrics_names = ["global accuracy", "weighted accuracy", "f1",
    ...
    label_metrics_names = ["accuracy", "f1", "precision", "recall",
                           "specificity", 'jaccard']
...
def wandb_mask(data, true_mask, labels, prediction=None):
    if prediction is not None:
        return wandb.Image(
            data,
            masks={"prediction": {"mask_data": prediction,
                                  "class_labels": labels},
