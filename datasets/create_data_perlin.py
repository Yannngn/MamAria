...
def shuffle_train_val(path, train_dir, val_dir, test_dir):
    ...
    lesions = np.unique(['_'.join(name.split('_')[:3]) for name in img_names])
    ...
    train_sort = [0]*a #random.choices(np.array([[i]*a for i in range(b)]).flatten(), k=a)
