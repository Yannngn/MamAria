...
FILES = ['perlin_spiculated_1_2']

def main(path = PATH, files = FILES, img_path = IMG_PATH, mask_path = MSK_PATH, get_slice = False, delete = False, shuffle = False):
    if delete:
        clear_data(img_path)
...
def get_slices(path, img_path = IMG_PATH, mask_path = MSK_PATH):
    ...
    for _, dirs, _ in os.walk(path):
        for folder in dirs:
            if folder[-4:] == "crop":
                slice_number = int(len(os.listdir(path + folder)) * .5)
                name = rename_image('_'.join(folder.split('_')[1:])[:-5], lesion)
                copyfile(path + folder + f"/_recon_slice_{slice_number}.tiff", 
                         img_path + lesion + '_' + '_'.join(name.split('_')) + ".tiff")

                copyfile(path + folder[:-5] + f"_mask/_{slice_number}.png", 
                         mask_path + lesion + '_' + '_'.join(name.split('_')) + "_mask.png")
                
def rename_image(name, lesion):
    print(name, lesion)
    lst = name.split('_')
    temp = lst[1].split('.')
    lst[1] = f'{int(temp[0]):02}' + f'{int(temp[1])*10}'[:2]
    if lst[3] == '1': lst[3] = '100'
    if lst[3] == '0.75': lst[3] = '075'
...
def shuffle_train_val(path, train_dir, val_dir):
    ...
    k = int(len(img_names) * .8)
    random.shuffle(img_names)
    train_names, val_names = img_names[:k], img_names[k:]
